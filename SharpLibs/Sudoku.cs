    ...
    public static class SudokuAlgorithms
    {
        public static IEnumerable<SudokuBoard> IterateAllAlgorithms(this SudokuBoard b)
        {
            while (true)
            {
                ...
                if (b.RemoveExtraOptionsFromBoundSquares())
                {
                    yield return b;
                    continue;
                }
        ...
        public static bool RestrictOptionsInGroupCommonToOtherGroup(this SudokuBoard board)
        {
            ...
            // rows with options in the same block:
            var cases = new List<(Func<SudokuSquare, int> primaryProperty,
                Func<int, IEnumerable<SudokuSquare>> primaryEnumerator,
                Func<SudokuSquare, int> secondaryProperty,
                Func<int, IEnumerable<SudokuSquare>> secondaryEnumerator)>
            {
                ((s) => s.Row, board.GetRow, (s) => s.Block, board.GetBlock),
                ((s) => s.Column, board.GetColumn, (s) => s.Block, board.GetBlock),
                ((s) => s.Block, board.GetBlock, (s) => s.Row, board.GetRow),
                ((s) => s.Block, board.GetBlock, (s) => s.Column, board.GetColumn),
            };

            foreach (var c in cases)
            {
                foreach (var i in SudokuIterators.Indexes)
                {
                    var group = c.primaryEnumerator(i);
                    var primary = c.primaryProperty(group.First());
                    foreach (var option in group.UnfoundOptions())
                    {
                        var squaresWithOption = group.Where(s => s.Options.Contains(option));
                        if (squaresWithOption.Select(c.secondaryProperty).Distinct().Count() == 1)
                        {
                            var secondary = c.secondaryProperty(squaresWithOption.First());
                            foreach (var s in c.secondaryEnumerator(secondary).Where(s => c.primaryProperty(s) != primary && !s.HasValue && s.Options.Contains(option)))
                            {
                                s.Forbid(option);
                                return true;
                            }
        ...
        /// <summary>
        /// If a N squares contain the same N options, then options other than
        /// those N can be removed from the square.
        /// </summary>
        public static bool RemoveExtraOptionsFromBoundSquares(this SudokuBoard b)
        {
            foreach (var enumerator in new[] {b.EnumerateRows(), b.EnumerateColumns(), b.EnumerateBlocks()})
            {
                foreach (var set in enumerator)
                {
                    if (set.All(s => s.HasValue))
                    {
                        continue;
                    }
                    for (var n = 2; n <= set.Count(s => !s.HasValue); n++)
                    {
                        foreach (var sq in set.Where(s => s.Options.Count == n))
                        {
                            var opts = sq.Options;
                            var commonSquares = set.Where(s => !s.HasValue && s.Options.Intersect(opts).Any());
                            if (commonSquares.Count() == n)
                            {
                                foreach (var cs in commonSquares)
                                {
                                    if (cs.Options.Count > n)
                                    {
                                        var removeOpts = cs.Options.Except(opts).ToList();
                                        foreach (var opt in removeOpts)
                                        {
                                            cs.Forbid(opt);
                                        }
                                        return true;
